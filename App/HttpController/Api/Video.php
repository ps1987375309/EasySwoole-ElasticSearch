...
class Video extends Base
{
    ...
    public function add() {
        ...
        $ruleObj->addColumn('name', '名字不为空')->required('名字不为空')->lengthMin(3,'最小长度不小于3位');
        ...
        $bool = $ruleObj->validate($data);
        if($bool == false) {
            //print_r($validata->getErrorList());
            return $this->writeJson(Status::CODE_BAD_REQUEST,$ruleObj->getError()->__toString());
        }
        ...
        if(!empty($videoId)) {
                return $this->writeJson(Status::CODE_OK, 'OK', ['插入状态' => $videoId ]);
        } else {
